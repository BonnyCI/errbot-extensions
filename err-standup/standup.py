...
from errbot import BotPlugin, botcmd  # pylint: disable=E0401


# pylint: disable=W0201
class Standup(BotPlugin):  # pylint: disable=R0904
    ...
    def initialize_scheduler(self):
        """ Timer-based init for the scheduler """
    ...
    def check_for_scheduled_standups(self):
        """
        This checks for any user who should be notified for their standup
        and notifies those users
        """
        tz_config = self.config.get('timezones', [])
        timezones = [group['timezone'] for group in tz_config]
        local_notify_hour = self.config.get('local_notification_hour', 10)
        msg = "{}: it's time for your standup! Use '!standup start' to begin"

        now = datetime.utcnow()
        for timezone in timezones:
            local_now = self.utc_to_timezone(now, timezone)
            users = self.get_local_users(timezone, tz_config)
            if local_now.weekday() < 5:  # M-F
                if local_now.hour == local_notify_hour:
                    self.notify_users(users, msg)
            else:
                self.clear_notified(users)

    @staticmethod
    def get_local_users(timezone, timezones):
        """ Returns a list of users for a given timezone """
        results = [group['users'] for group in timezones
    ...
    def notify_users(self, users, msg):
        """ Notify a list of users """
        for user in users:
            if self.notified.get(user, False) is False:
                try:
    ...
    def clear_notified(self, users):
        """ Clear notification flag for a list of users """
        for user in users:
            self.notified[user] = False

    @staticmethod
    def utc_to_timezone(a_date, a_tz):
        """ Change a utc time to a local timezone """
        utc = pytz.timezone('UTC')
        return a_date.replace(tzinfo=utc).astimezone(pytz.timezone(a_tz))

    # Bot commands section

    @botcmd
    def standup(self, msg, args):
        """ Alias for '!standup help' """
        return self.standup_help(msg, args)

    @botcmd
    def standup_help(self, msg, args):  # pylint: disable=W0613,R0201
        """Gives the user info on how to use the standup plugin
           usage: !standup help"""
        lines = [("!standup start                             "
                  "-- Start today's standup"),
                 ("!standup yesterday/today/blockers <status> "
                  "-- Add to today's standup"),
                 ("!standup review                            "
                  "-- Review today's uncommitted standup"),
                 ("!standup commit                            "
                  "-- Commit today's standup"),
                 ("!standup log                               "
                  "-- Show today's committed standup for your user"),
                 ("!standup delete <id>                       "
                  "-- Delete today's standup"),
                 ("!standup team <date>                       "
                  "-- Show team's standup for date (default = today)")]
        return '\n'.join(lines)

    @botcmd
    def standup_start(self, msg, args):  # pylint: disable=W0613,R0201
        """Start staging a standup
           usage: !standup start"""
        user = msg.frm.nick
        self.clear_stage(user)
        return ("Please use '!standup yesterday/today/blockers' "
                "to stage your standup, "
                "and '!standup commit' when you're done")

    def clear_stage(self, user):
        """ Helper to clear the staging area for a given user """
        self.staged[user] = {}

    @botcmd
    def standup_yesterday(self, msg, args):
        """ Bot command to set 'yesterday' in a staged standup """
        return self.standup_set_part(msg.frm.nick, 'yesterday', args)

    @botcmd
    def standup_today(self, msg, args):
        """ Bot command to set 'today' in a staged standup """
        return self.standup_set_part(msg.frm.nick, 'today', args)

    @botcmd
    def standup_blockers(self, msg, args):
        """ Bot command to set 'blockers' in a staged standup """
        return self.standup_set_part(msg.frm.nick, 'blockers', args)

    def standup_set_part(self, user, part, status):
        """ Stage a part of a user's standup """
    ...
    def get_staging(self, user):
        """ Fetch a user's staging state
            Users stage their standup before committing it with
            '!standup commit' """
        return self.staged.get(user, None)

    @botcmd
    def standup_review(self, msg, args):  # pylint: disable=W0613
        """ Review a standup before committing it
            usage: !standup review """
        user = msg.frm.nick
    ...
    def get_local_date_for_user(self, user, timezones):
        """ Fetch the local date for a user """
        timezone = self.get_timezone_for_user(user, timezones)
        if timezone is None:
    ...
    @botcmd
    def standup_commit(self, msg, args):  # pylint: disable=W0613
        """Adds a new status
        ...
        for part in ['yesterday', 'today', 'blockers']:
            if part not in staged:
                return ("{}: not all field filled. "
                        "Use '!standup review' to determine "
                        "which fields are missing").format(part)
        local_date = self.get_local_date_for_user(user,
                                                  self.config["timezones"])
        existing_statuses = self.db_get_status_by_author_date(self.con,
                                                              user,
                                                              local_date)
        if len(existing_statuses) > 0:
            return ("Oops, previous standup already committed for today."
                    "Please use '!standup delete' to remove prior standup")
        else:
            self.db_insert_status(self.con, user, staged, local_date)
            self.clear_stage(user)
            return ("Standup committed."
                    "Use '!standup log' to see committed standup,"
                    "or '!standup delete' to redo today's standup")

    @staticmethod
    def get_timezone_for_user(user, timezones):
        """ For a given user, fetch its timezone """
        results = [group['timezone'] for group in timezones
    ...
    def db_insert_status(db_conn, author, status, date):
        """ Insert a new standup into the database """
        yesterday = status['yesterday']
        today = status['today']
        blockers = status['blockers']
        with db_conn as conn:
            conn.execute("""INSERT INTO statuses
                            (yesterday, today, blockers, author, date)
                            VALUES (?,?,?,?,?)""",
                         (yesterday, today, blockers, author, date))
    ...
    def standup_log(self, msg, args):
        ...
        statuses = self.db_get_status_by_author_date(self.con, user, date)
    ...
    @staticmethod
    def db_get_status_by_author_date(db_conn, author, date):
        """ Lookup standups that match a specific user and date """
        cur = db_conn.cursor()
        cur.execute("""SELECT id, yesterday, today, blockers
                       FROM statuses
                       WHERE author=? AND date=?""", (author, date))
        results = [{'id': row[0],
                    'yesterday': row[1],
                    'today': row[2],
    ...
    def standup_delete(self, msg, args):
        ...
        local_date = self.get_local_date_for_user(user,
                                                  self.config["timezones"])
        count = self.db_delete_status_by_id(self.con,
                                            status_id,
                                            user,
                                            local_date)
        if count > 0:
    ...
    def db_delete_status_by_id(db_conn, status_id, author, date):
        """ Delete a given status by its unique id """
        cur = db_conn.cursor()
        cur.execute("""DELETE FROM statuses
                       WHERE id=? AND author=? AND date=?""",
                    (status_id, author, date))
        db_conn.commit()
        return cur.rowcount
    ...
    def db_get_statuses_from_date(db_conn, date):
        """ Lookup standups for a given date """
        cur = db_conn.cursor()
        cur.execute("""SELECT author, yesterday, today, blockers
                       FROM statuses
                       WHERE date=?""", (date, ))
